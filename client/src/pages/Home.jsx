    ...
    <div className="animate-fade-in">
      ...
      <section style={{ padding: '6rem 1.5rem', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          ...
          <h1 style={{ fontSize: '4rem', marginBottom: '1.5rem', lineHeight: 1.1, color: '#db834d', textShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>
            Bridge the gap between <br />
            <span style={{ color: '#db834d', WebkitTextFillColor: '#db834d' }}>life and hope.</span>
          </h1>
          ...
          {!user ? (
            <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
              <Link to="/register" className="btn btn-hero-nav" style={{ padding: '1rem 2rem', fontSize: '1.05rem', fontWeight: 600 }}>Become a Donor</Link>
              <Link to="/register" className="btn btn-hero-nav" style={{ padding: '1rem 2rem', fontSize: '1.05rem', fontWeight: 600 }}>Request an Organ</Link>
            </div>
          ) : (
            <Link to={user.role === 'donor' ? '/dashboard/donor' : user.role === 'recipient' ? '/dashboard/recipient' : `/dashboard/${user.role}`} className="btn btn-hero-nav" style={{ padding: '1rem 2rem', fontSize: '1.05rem', fontWeight: 600 }}>
              Go to Dashboard
            </Link>
          )}
        </div>
      ...
      <section style={{ padding: '5rem 1.5rem', backgroundColor: 'transparent' }}>
        <div className="container">
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            <h2 style={{ fontSize: '2.5rem', marginBottom: '1rem', color: '#000000', fontWeight: 800 }}>How JeevanSetu Works</h2>
            <p style={{ color: '#000000', fontSize: '1.05rem', fontWeight: 600 }}>Our platform ensures transparency, speed, and security at every step.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
            {[
              ...
              <div key={idx} style={{ padding: '2.5rem 2rem', textAlign: 'center', background: '#db834d', border: 'none', borderRadius: '20px', boxShadow: 'var(--shadow-sm)', transition: 'transform 0.2s, box-shadow 0.2s' }}
              ...
              >
                ...
                <h3 style={{ fontSize: '1.25rem', marginBottom: '1rem', color: '#000000', fontWeight: 700 }}>{feature.title}</h3>
                <p style={{ color: '#000000', lineHeight: 1.7, fontWeight: 500 }}>{feature.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem', textAlign: 'center' }}>
            {[
              ...
              <div key={i}>
                <div style={{ fontSize: '2.5rem', fontWeight: 800, color: '#000000', fontFamily: 'var(--font-display)', marginBottom: '0.5rem' }}>{stat.value}</div>
                <div style={{ color: '#000000', fontSize: '0.95rem', fontWeight: 600 }}>{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
